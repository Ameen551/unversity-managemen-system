...
function detectField(header: string): keyof ImportRow | undefined {
  if (headerMap.has(header)) return headerMap.get(header);
  if (header.includes('student id') || header.includes('roll number') || header.includes('roll no')) return 'studentId';
  if (header.includes('father')) return 'fatherName';
  if (header.includes('student name') || header === 'name') return 'name';
  if (header.includes('section')) return 'section';
  return undefined;
}

/** Downloads: generate a sample Excel/CSV template for student import. */
export async function buildStudentTemplate(): Promise<ExcelJS.Buffer> {
...
function normalizeRows(rows: unknown[][]): ImportRow[] {
  ...
  const mapped: { colIndex: number; field: keyof ImportRow }[] = [];
  for (let i = 0; i < headers.length; i++) {
    const key = detectField(headers[i]);
    if (key) mapped.push({ colIndex: i, field: key });
  }
  if (mapped.length < 2) {
    throw errors.unprocessable(
      'Invalid file format. The file must contain columns: Student Name, Father Name, Student ID / Roll Number, Section.'
    );
  }
  ...
  for (let r = 1; r < rows.length; r++) {
    const raw = rows[r] ?? [];
    const cell = (col: { colIndex: number } | undefined): string => {
    ...
    if (!name && !fatherName && !studentId) continue; // skip fully empty rows
 ...
 */
export async function readUploadRows(buffer: Buffer, filename: string): Promise<unknown[][]> {
...
export async function importStudents(
  ...
  const result: ImportResult = { successCount: 0, errorCount: 0, duplicateCount: 0, issues: [] };
  ...
  if (subjects.length > 0) {
    ...
    const data = importedStudents.flatMap((s) =>
      subjects
...
export async function buildReportWorkbook(sheetName: string, columns: string[], rows: ReportRow[]): Promise<ExcelJS.Buffer> {
  const workbook = new ExcelJS.Workbook();
...
export function buildCsvReport(columns: string[], rows: ReportRow[]): string {
