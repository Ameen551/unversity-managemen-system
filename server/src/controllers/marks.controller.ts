...
export const upsertMarks = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const rec of data.records) {
    if (rec.obtainedMarks > rec.totalMarks) {
      throw errors.badRequest(
        `Obtained marks (${rec.obtainedMarks}) cannot exceed total marks (${rec.totalMarks}).`
      );
    }
...
export const updateMarks = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existing) throw errors.notFound('Marks record not found.');
...
export const listAssessmentConfigs = asyncHandler(async (_req: Request, res: Response) => {
