...
export const listSubjects = asyncHandler(async (req: Request, res: Response) => {
  const { departmentId, sessionId, semesterId } = req.query as Record<string, string | undefined>;
  const where: Record<string, unknown> = {};
  if (departmentId) where.departmentId = Number(departmentId);
  if (sessionId) where.sessionId = Number(sessionId);
  if (semesterId) where.semesterId = Number(semesterId);
  if (!isAdminRole(req.user!.role) || req.query.all !== 'true') where.isActive = true;
...
export const updateSubject = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!existing) throw errors.notFound('Subject not found.');
