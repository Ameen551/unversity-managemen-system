...
export const getStudent = asyncHandler(async (req: Request, res: Response) => {
  ...
  const summaryMap = new Map<number, { total: number; present: number; absent: number; leave: number }>();
...
async function ensureScopeValid(data: z.infer<typeof studentSchema>): Promise<void> {
...
export const createStudent = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (existing) {
    throw errors.conflict(
      `Student ID "${data.studentId}" already exists. Please use a unique ID.`
    );
  }
...
export const updateStudent = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (data.studentId && data.studentId !== existing.studentId) {
    ...
    if (clash) throw errors.conflict(`Student ID "${data.studentId}" already exists.`);
  }
  ...
  if (data.departmentId || data.sessionId || data.semesterId) {
    ...
    await ensureScopeValid(merged as z.infer<typeof studentSchema>);
  }
...
export const deleteStudent = asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = await prisma.student.findUnique({ where: { id } });
  if (!existing) throw errors.notFound('Student not found.');
  if (existing.isDeleted) throw errors.badRequest('Student is already deleted.');
...
export const restoreStudent = asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = await prisma.student.findUnique({ where: { id } });
  if (!existing) throw errors.notFound('Student not found.');
  if (!existing.isDeleted) throw errors.badRequest('Student is not deleted.');
