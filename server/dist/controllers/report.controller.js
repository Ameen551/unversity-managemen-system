...
import { buildCsvReport, buildReportWorkbook } from '../services/excel.service';
function resolveFormat(q) {
    const f = q.format;
    if (f === 'csv')
        return 'csv';
    return 'excel';
}
function respondReport(res, filename, format, columns, rows) {
    if (format === 'csv') {
...
export const exportMarks = asyncHandler(async (req, res) => {
    const q = req.query;
    ...
    const rows = subjects.flatMap((sub) => sub.marks.map((m) => ({
...
export const exportAttendance = asyncHandler(async (req, res) => {
    ...
    const format = resolveFormat(q);
    if (!subjectId)
        throw errors.badRequest('Provide a subjectId to export attendance.');
...
export const exportOverall = asyncHandler(async (req, res) => {
    const q = req.query;
    ...
    const rows = students.map((s) => {
        ...
        const sum = (arr) => arr.reduce((a, b) => a + b.obtainedMarks, 0);
        const sumTotal = (arr) => arr.reduce((a, b) => a + b.totalMarks, 0);
