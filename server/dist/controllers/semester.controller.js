...
export const listSemesters = asyncHandler(async (req, res) => {
    ...
    if (!departmentId)
        throw errors.badRequest('departmentId query parameter is required.');
...
export const createSemester = asyncHandler(async (req, res) => {
    ...
    if (existing) {
        throw errors.conflict(`Semester ${data.number} already exists for ${department.name}.`);
    }
...
export const updateSemester = asyncHandler(async (req, res) => {
    ...
    if (data.departmentId && data.number) {
        ...
        if (clash)
            throw errors.conflict('That semester number already exists for the department.');
    }
...
export const bulkCreateSemesters = asyncHandler(async (req, res) => {
